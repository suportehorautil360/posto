...
export function ChdListPage() {
  ...
  const loadChecklists = useCallback(async () => {
    ...
    try {
      const data = await getChecklistsDevolucaoOficina(oficina.id);
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : chdListPageConfig.states.error
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-brand-navy">
                {chdListPageConfig.title}
              </h1>
              <p className="mt-1 text-sm text-zinc-500">
                {chdListPageConfig.subtitle}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-zinc-200/80 bg-white shadow-sm">
        {!oficina?.id ? (
          <div className="px-6 py-12 text-center text-sm text-zinc-500">
            {chdListPageConfig.states.noOficina}
          </div>
        ) : isLoading ? (
          <div className="px-6 py-12 text-center text-sm text-zinc-500">
            {chdListPageConfig.states.loading}
          </div>
        ) : errorMessage ? (
          <div className="flex flex-col items-center gap-4 px-6 py-12 text-center">
            ...
            >
              {chdListPageConfig.actions.retry}
            </button>
          </div>
        ) : checklists.length === 0 ? (
          <div className="px-6 py-12 text-center">
            <p className="text-base font-semibold text-brand-navy">
              {chdListPageConfig.empty.title}
            </p>
            <p className="mt-2 text-sm text-zinc-500">
              {chdListPageConfig.empty.description}
            </p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{chdListPageConfig.columns.number}</TableHead>
                <TableHead>{chdListPageConfig.columns.equipment}</TableHead>
                <TableHead>{chdListPageConfig.columns.driver}</TableHead>
                <TableHead>{chdListPageConfig.columns.status}</TableHead>
                <TableHead>{chdListPageConfig.columns.date}</TableHead>
                <TableHead className="text-right">
                  {chdListPageConfig.columns.actions}
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {checklists.map((checklist) => (
                <TableRow key={checklist.id}>
                  ...
                  <TableCell>{checklist.identification.driver || "—"}</TableCell>
                  <TableCell>{formatStatusLabel(checklist.status)}</TableCell>
                  <TableCell>
